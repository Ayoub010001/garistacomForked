...
function App() {
  ...
  const fetchDishes = useCallback(
    async (restoId) => {
      try {
        ...
        // Parse responses
        const dishesData = await (dishesResponse.ok
          ? dishesResponse.json()
          : dishesResponse.json().then((data) => data));
        const drinksData = await (drinksResponse.ok
          ? drinksResponse.json()
          : drinksResponse.json().then((data) => data));
        ...
        if (dishesData && dishesData.length) {
          combinedData.push(
            ...dishesData.map((item) => ({ ...item, type: "dish" }))
          );
        }
        ...
        if (drinksData && drinksData.length) {
          combinedData.push(
            ...drinksData.map((item) => ({ ...item, type: "drink" }))
          );
        }
        ...
        if (
          ...
          console.log(
            "No matching dishes or drinks found or specific message received."
          );
          // Optionally set a state to show this message in the UI
          setMessage(
            dishesData.message || drinksData.message || "No items found."
          );
        }
      } catch (error) {
  ...
  const fetchRestosbyslug = useCallback(async () => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    const fetchRestaurantData = async () => {
      await fetchRestosbyslug();
      await fetchDishes(restoId);
    };

    fetchRestaurantData();
  }, [fetchDishes, fetchRestosbyslug, restoId]); // Fetch restos when the component mounts

  useEffect(() => {
    if (selectedTab) {
      fetchDishes(restoId); // Fetch dishes when selectedTab changes
    }
  }, [fetchDishes, restoId, selectedTab]);
  if (loading) {
    return (
